...
Array<int> ar(5, 3);
```
Вроде на первый взгляд всё хорошо, но это не так. При вызове конструктора для `ar(5, 3)` компилятору сложно понять: а какой именно конструктор звать-то? Внезапно, выясняется, что конструктор, принимающий итераторы будет предпочтительнее, ибо в нём не нужно кастовать `5` к `size_t`.
